...
export default function VerifyEmailPage() {
  ...
  return <main className={styles.shell}><section className={styles.card}>
    ...
    <div style={{ marginTop: 20 }}><label className={styles.form[0] as never}> </label><input style={{ width: "100%", minHeight: 48, padding: "0 15px", borderRadius: 14, border: "1px solid rgba(16,34,31,.11)" }} type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email for resend" /><button className="button secondary full" style={{ marginTop: 10 }} disabled={busy || !email} onClick={resend} type="button">Resend verification</button></div>
