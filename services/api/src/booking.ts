...
export async function registerBookingRoutes(app: FastifyInstance, allowDemoAuth: boolean) {
  app.get("/v1/doctors/:slug/availability", async (request, reply) => {
    ...
    return slots.map((slot) => ({ id: slot.id, startsAt: slot.startsAt.toISOString(), endsAt: slot.endsAt.toISOString(), timezone: "UTC" }));
  });
  ...
  app.post("/v1/appointments", async (request, reply) => {
    ...
    if (typeof demoPatientEmail !== "string" || !demoPatientEmail.endsWith("@demo.carebridge.local")) {
    ...
    if (!patientUser || patientUser.role !== "PATIENT") return reply.code(401).send({ error: "Demo patient is not registered" });
    ...
    try {
      const appointment = await prisma.$transaction(async (tx) => {
        ...
        return tx.appointment.create({
          data: { patientId: patient.id, doctorId: doctor.id, scheduledAt: input.scheduledAt, durationMin: input.durationMin, status: "BOOKED" },
          select: { id: true, scheduledAt: true, durationMin: true, status: true },
        });
      });
      return reply.code(201).send({ id: appointment.id, scheduledAt: appointment.scheduledAt.toISOString(), durationMin: appointment.durationMin, status: appointment.status });
    } catch (error) {
      if (error instanceof Error && error.message === "SLOT_ALREADY_BOOKED") return reply.code(409).send({ error: "Selected time is no longer available" });
      throw error;
    }
  });
}
