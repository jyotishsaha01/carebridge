...
const env = z.object({ PORT: z.coerce.number().int().positive().default(4000), HOST: z.string().default("0.0.0.0"), DATABASE_URL: z.string().min(1), CORS_ORIGIN: z.string().default("http://localhost:3000"), ALLOW_DEMO_AUTH: z.string().default("false").transform((value) => value === "true") }).parse(process.env);
